(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const t of o)if(t.type==="childList")for(const i of t.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(o){const t={};return o.integrity&&(t.integrity=o.integrity),o.referrerPolicy&&(t.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?t.credentials="include":o.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function a(o){if(o.ep)return;o.ep=!0;const t=n(o);fetch(o.href,t)}})();class c extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
      :host {
      }
...
`;class p extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return h}connectedCallback(){this.render();const[e,...n]=this.shadowRoot.querySelectorAll("input[type=text], select:not([disabled])");e.addEventListener("input",()=>this.updateURL()),n.forEach(a=>{a.addEventListener("change",()=>this.updateURL())}),this.shadowRoot.querySelector("button").addEventListener("click",()=>this.copyURL())}async copyURL(){const e=this.shadowRoot.querySelector("textarea").value;await navigator.clipboard.writeText(e)}getURL(){const e=new URL(location.href).origin,[n,...a]=[...this.shadowRoot.querySelectorAll("input[type=text], select:not([disabled])")],o=n.value,t=a.map(i=>`${i.name}=${i[i.selectedIndex].value}`).join("&");return`${e}?channel=${o.toLowerCase()}&${t}`}updateURL(){const e=this.shadowRoot.querySelector("textarea");e.value=this.getURL()}render(){this.shadowRoot.innerHTML=`
    ...
    </div>`}}customElements.define("screen-menu",p);class f{constructor(){const{searchParams:e}=new URL(location.href),n=e.get("channel")??"ManzDev",a=e.get("mode")??"bit";this.diploPollo=document.querySelector("diplo-pollo"),this.connect(n),a==="bit"&&this.enableBitMode(),a==="command"&&this.enableCommandMode()}connect(e){this.client=new tmi.Client({channels:[e]}),this.client.connect()}enableBitMode(){this.client.on("cheer",(e,n,a)=>{const o=n.bits??1,t=Math.floor(o/2);this.diploPollo.incNeck(t)})}enableCommandMode(){this.client.on("message",(e,n,a,o)=>{if(!(a[0]==="!"))return;if(a.split(" ").at(0).toLowerCase()==="!pollo"){const l=Number.parseInt(a.split(" ").at(1)??1);if(!(!isNaN(l)&&l>-300&&l<300))return;console.log(l),l>0?this.diploPollo.incNeck(l):l<0&&this.diploPollo.decNeck(Math.abs(l))}})}}const r=document.querySelector(".container"),{searchParams:g}=new URL(location.href),m=!!g.get("channel");if(m){const s=document.createElement("diplo-pollo");r.classList.add("diplopollo-mode"),s.classList.add("animated"),r.append(s),new f}if(!m){r.classList.add("menu-mode");const s=document.createElement("screen-menu");r.append(s)}
